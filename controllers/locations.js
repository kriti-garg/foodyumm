  ...
  (err,response, bode)=> {
    ...
    if(response.statusCode === 200 && data.length){
     for(let i=0;i<data.length;i++) {
       data[i].distance=formatDistence(data[i].distance);
     }
    }
...
const locationsListByfranchiseName = function (req, res) {
  if (req.params && req.params.locationid) {
    Loc
      .findById( { franchiseName:"Burgrill" } )
      .exec((err, location) => {
        if (!franchiseName) {
          res	
            .status(404) 
            .json({	
              "message": "franchise menu not found"
            });	 
          return;
        } else if (err) {
          res	
